...
class PCNN(Base):
	...
	def add_model(self):
		...
		with tf.variable_scope('Convolution') as scope:
			...
			kernel = tf.get_variable('kernel', [self.p.ngram, conv_in_dim, 1, self.p.num_filters], 	initializer=tf.truncated_normal_initializer(),  regularizer=self.regularizer)
			biases = tf.get_variable('biases', [self.p.num_filters], 				initializer=tf.random_normal_initializer(), 	regularizer=self.regularizer)
		...
		with tf.variable_scope('FC1') as scope:
			w_rel   = tf.get_variable('w_rel', [self.p.num_filters, self.num_class], 	initializer=tf.truncated_normal_initializer(),	regularizer=self.regularizer)
...
if __name__== "__main__":
	...
	parser.add_argument('-rel2id_map',     	default='../data/inell_relations.json', 						help='File containing relation to id mapping')
